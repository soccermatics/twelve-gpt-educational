...
from classes.embeddings import PlayerEmbeddings, PersonEmbeddings, CountryEmbeddings
...
class Chat:
    ...
    def get_input(self):
        ...
        if x := st.chat_input(
            placeholder=f"What else would you like to know?"
        ):
            if len(x) > 500:
                st.error(
...
class PersonChat(Chat):
    ...
    def instruction_messages(self):
        ...
        def get_input(self):
            ...
            if x := st.chat_input(
                placeholder=f"What else would you like to know about the person?"
            ):
                if len(x) > 500:
                    st.error(
                        f"Your message is too long ({len(x)} characters). Please keep it under 500 characters."
